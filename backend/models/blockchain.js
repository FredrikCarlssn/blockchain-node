...
class Blockchain {
	constructor() {
		...
		this.networkNodes = [];
		console.log(`This m** f** is rich as f**, go steal his money: ${this.ledger.wallets[0].address}`);
	}
	...
	chainIsValid(blockchain) {
		const isValid = blockchain.every((block, index) => {
			if (index === 0) {
				if (block.hash !== "Genesis" || block.previousBlockHash !== "Genesis" || block.data) {
					return false;
				} else {
					return true;
				}
			...
			const generatedHash = this.hashBlock(previousBlockHash, currentBlockData, nonce);
	...
	tokenTransaction(transaction) {
		const senderWallet = this.ledger.wallets.find((wallet) => wallet.address === transaction.fromAddress);
		const recipientWallet = this.ledger.wallets.find((wallet) => wallet.address === transaction.toAddress);
	...
	validateTokenTransaction(transaction) {
		...
		if (!transaction.toAddress || !senderWallet || senderWallet.balance < transaction.amount) {
			return false;
		}
